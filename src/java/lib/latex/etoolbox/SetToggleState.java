...
{
   public SetToggleState(boolean state)
   {
      this("toggle"+state, state);
   }

   public SetToggleState(String name, boolean state)
   {
      super(name);
      this.state = state;
   }
   ...
   public Object clone()
   {
      return new SetToggleState(getName(), state);
   }
     ...
     throws IOException
   {
      ...
      if (state)
      {
         parser.putControlSequence(new AtFirstOfTwo(csname));
      }
      else
      {
